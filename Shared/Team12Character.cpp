...
void ATeam12Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		//DEBUG!//
		EnhancedInputComponent->BindAction(MutationAction, ETriggerEvent::Started, this, &ATeam12Character::SpawnMutationDebug);
		
		// TODO: Make this gamepad too?
...
// ATTENTION! THIS IS A DEBUG METHOD AND IS NOT TO BE RELEASED WITH THE GAME OR ANY GAME BUILDS!

void ATeam12Character::SpawnMutationDebug() 
{
	ATeam12Character* Player = this;
	if (Player != nullptr)
	{
		...
		if (GameMode != nullptr)
		{
			// Access the variables or functions of the game mode
			if (GameMode->Mutations.Num() > 0)
			{
				// Generate a random index within the array bounds
				int32 RandomIndex = FMath::RandRange(0, GameMode->Mutations.Num() - 1);

				// Retrieve the random element
				TSubclassOf<AActor> ActorClass = GameMode->Mutations[RandomIndex];

				if (ActorClass->IsChildOf(AMutationCluster::StaticClass()))
				{
					AMutationCluster* Mutation = Cast<AMutationCluster>(ActorClass.GetDefaultObject());
					...
					if (Mutation != nullptr)
					{
						...
						GetWorld()->SpawnActor<AMutationCluster>(ActorClass, FTransform::Identity, SpawnParams);
						if (Mutation != nullptr)
						{
